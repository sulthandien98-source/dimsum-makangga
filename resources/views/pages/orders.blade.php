...
<div class="max-w-4xl mx-auto px-4">
    ...
    <div x-data="{show:true}" x-init="setTimeout(() => show = false, 3500)" x-show="show"
        ...
        <button @click="show=false" class="text-green-400/60 hover:text-green-400">✕</button>
    </div>
    @endif
    ...
    <div class="space-y-4">
        ...
        <div class="card">
            ...
            <div class="flex items-start justify-between mb-3 gap-2 flex-wrap">
                <div>
                    <div class="flex items-center gap-2 flex-wrap">
                        <span class="text-white font-black text-base">#{{ $o->id }}</span>
                        <span class="text-amber-100/80 font-semibold">{{ $o->customer_name }}</span>
                    </div>
                ...
                <div class="flex flex-col items-end gap-1.5">
                    ...
                    @php
                        ...
                        $oBadge = $orderBadge[$o->status] ?? 'bg-gray-500/20 text-gray-400 border-gray-500/30';
                    @endphp
            ...
            <div class="border-t border-white/5 py-3 text-sm text-amber-200/60 space-y-1">
                ...
                <div class="flex justify-between">
                    <span>{{ $item->product->name ?? 'Produk dihapus' }} ×{{ $item->quantity }}</span>
            ...
            <div class="flex justify-between items-center pt-3 border-t border-white/5">
                <span class="text-amber-200/50 text-sm">Total</span>
            ...
            <div class="mt-3 bg-red-500/10 border border-red-500/20 rounded-xl p-4">
                ...
                <p class="text-red-300 text-sm">{{ $o->rejection_reason }}</p>
            ...
            {{-- UPLOAD BUTTON if pending/rejected --}}
            @if(!$o->isOrderRejected() && (!$o->hasPaymentProof() || $o->isPaymentRejected()))
            <div class="mt-3">
                <a href="{{ route('payment.proof', $o->id) }}"
                   class="block w-full text-center py-2.5 bg-orange-500 hover:bg-orange-600 transition
        ...
        <div class="card text-center py-16">
            ...
            <p class="text-amber-200/50 mb-4">Belum ada pesanan</p>
            <a href="{{ route('menu') }}"
